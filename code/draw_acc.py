...
y = np.load('data_acc.npy')
df = pd.DataFrame(y)
                  
data = split(y)

time = np.array(data[0])
gyro_xout = np.array(data[1])  
gyro_yout = np.array(data[2])  
gyro_zout = np.array(data[3])  

accel_xout = np.array(data[4])  
accel_yout = np.array(data[5])  
accel_zout = np.array(data[6])  

x_rotation = np.array(data[7])  
y_rotation = np.array(data[8])  

data1 = interpolate(gyro_zout)
data2 = interpolate(accel_zout)
...
plt.xlabel('Time')
plt.ylabel('accel_zout')
plt.title('accel_zout')
